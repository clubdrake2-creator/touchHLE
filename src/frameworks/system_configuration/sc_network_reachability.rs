...
fn SCNetworkReachabilityGetFlags(
    ...
    let known_class = env.objc.get_known_class("_touchHLE_SCNetworkReachability", &mut env.mem);
    
    assert_eq!(target_class, known_class);

    // SMASH FIX: Always report Reachable and IsDirect (WiFi/Direct connection).
    // Old iOS games often disable UI buttons if this returns false or 0 flags.
    let out_flags = kSCNetworkReachabilityFlagsReachable | kSCNetworkReachabilityFlagsIsDirect;
    
    log!("SCNetworkReachabilityGetFlags [HyperHLE Fix]: Forcing Reachable state for SMASH.");
...
fn SCNetworkReachabilitySetCallback(
    ...
    log!(
        "SCNetworkReachabilitySetCallback [SMASH FIX]: Faking success for {:?}, {:?}, {:?}",
        target,
        callout,
        context
    );
    // Return true so the game thinks the network observer is active
    true
}
